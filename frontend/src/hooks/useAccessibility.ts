...
export const useAccessibleChart = () => {
  ...
  const generateChartDescription = useCallback((
    ...
    const description = i18n.language === 'de' 
      ? `${chartType === 'pie' ? 'Kreisdiagramm' : chartType === 'line' ? 'Liniendiagramm' : 'Balkendiagramm'}: ${title}. ${data.length} Datenpunkte. H�chster Wert: ${maxItem?.label} mit ${max}. Gesamtwert: ${total}.`
  ...
  const getChartKeyboardInstructions = useCallback(() => {
    return i18n.language === 'de'
      ? 'Verwenden Sie die Pfeiltasten, um durch die Datenpunkte zu navigieren. Dr�cken Sie Enter f�r Details.'
...
export const useAccessibleModal = (isOpen: boolean, title: string) => {
  ...
  useEffect(() => {
    if (isOpen) {
      saveFocus();
      announce(
        i18n.language === 'de' ? `Dialog ge�ffnet: ${title}` : `Dialog opened: ${title}`,
        'assertive'
      );
