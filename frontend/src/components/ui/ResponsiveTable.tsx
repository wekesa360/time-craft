...
function ResponsiveTable<T>({
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    const message = language === 'de'
      ? `${checked ? 'Alle' : 'Keine'} Zeilen ausgew�hlt`
        ...
        >
          {displayedData.map((item, index) => (
            <div
              ...
              aria-label={
                onRowClick 
                  ? (language === 'de' ? `Zeile ${index + 1} ausw�hlen` : `Select row ${index + 1}`)
                  : undefined
              }
            >
              {selectable && (
    ...
    <div className={cn('responsive-table-container', className)}>
        ...
        <div className="mb-4 flex items-center space-x-2">
          ...
          {sortedData.length > 0 && (
            <span className="text-sm text-foreground-secondary">
              {language === 'de' 
                ? `${sortedData.length} von ${data.length} Eintr�gen`
              ...
              {selectable && (
                ...
                >
                  <input
                    ...
                    aria-label={
                      language === 'de'
                        ? 'Alle Zeilen ausw�hlen/abw�hlen'
      ...
      {displayedData.length === 0 && (
        <div className="text-center py-8">
          <p className="text-foreground-secondary">
            {emptyMessage || 
              (language === 'de' ? 'Keine Daten verf�gbar' : 'No data available')
            }
          </p>
        </div>
      )}
      ...
      {pagination && totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-foreground-secondary">
            {language === 'de'
              ? `${startIndex + 1}-${Math.min(endIndex, sortedData.length)} von ${sortedData.length} Eintr�gen`
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="btn-secondary btn-sm"
              aria-label={language === 'de' ? 'N�chste Seite' : 'Next page'}
            >
      ...
      <div className="sr-only" aria-live="polite">
        {language === 'de' ? (
          ...
          ${selectable ? 'Leertaste zum Ausw�hlen.' : ''}`
        ) : (
